 ...
 * @copyright Copyright (c) 2021 WarpTheme
...
class SppagebuilderAddonUiSlideShow extends SppagebuilderAddons {
	...
	public function render() {
		...
		// New 
		$button_font_weight      = ( isset( $settings->button_font_weight ) && $settings->button_font_weight ) ? ' uk-text-' . $settings->button_font_weight : '';
		$button_text_transform   = ( isset( $settings->button_text_transform ) && $settings->button_text_transform ) ? ' uk-text-' . $settings->button_text_transform : '';

		$button_style_cls .= $button_font_weight . $button_text_transform;
		...
		$output .= '<div class="tm-slideshow-wrapper' . $zindex_cls . $general . $max_width_cfg . '"' . $animation . '>';
		...
		if ( isset( $settings->ui_slideshow_items ) && count( (array) $settings->ui_slideshow_items ) ) {
			foreach ( $settings->ui_slideshow_items as $key => $value ) {
				...
				$video_item = ( isset( $value->video_item ) && $value->video_item ) ? $value->video_item : '';
				...
				if($image_src) {
					...
					$output .= '<video class="el-image uk-cover" uk-cover="" src="'.$video_item.'" loop autoplay muted playsinline></video>';
				}
