...
if (!empty($visibleLists)) {
    ...
    foreach ($visibleLists as $myListId) {
        ...
        $listsContent .= '
            <div class="onelist">
                <label for="acylist_'.$myListId.'_'.$formName.'"><input type="checkbox" class="uk-checkbox" name="subscription[]" id="acylist_'.$myListId.'_'.$formName.'" '.$check.' value="'.$myListId.'"/> '.$allLists[$myListId]->name.'</label>
            </div>';
    }
    $listsContent .= '</div>';
}
...
<div class="tm-acym-form uk-grid-small uk-child-width-expand@s uk-flex uk-flex-middle" uk-grid>   
    ...
    <input type="button"
	   class="uk-button uk-button-default subbutton"
...
if (empty($identifiedUser->id) && $config->get('captcha', '') == 1) {
    echo '<div class="onefield fieldacycaptcha uk-margin" id="field_captcha_'.$formName.'">';
